...
**Legend**
- **DECIDE** — open product call; a recommendation is given, meeting confirms. All: #2, #12, #13, #16, #19, #22, #25, #27, #57, #58, #89.
- **❓** — feedback too vague to fully pin down; action is a best guess, open question at the bottom. All: #42, #43, #44, #47, #51, #52.
...
| 2 | Opportunity for the app to just be a progressive web app you add from Safari, as well as an app you install? To look at after the meeting. | **DECIDE** (after meeting): also ship as an installable PWA. Rec: yes — cheap distribution while TestFlight-gated. | Mobile app |
| 3 | On browser, after you close the install app link, goes away forever? Maybe should reappear on refresh? | Re-show the dismissed install banner on a later visit (e.g. next session or after ~7 days), not never. | Mobile web |
| 4 | On browser, where does the install app link go right now, since it's TestFlight? | Audit where the link points today; make it open the TestFlight invite, labeled as beta, until App Store release. | Mobile web |
...
| 7 | In the browser (mobile chrome) version, when I scroll down, url bar stays there so it never enters full screen like with other pages. | Make scrolling collapse the browser URL bar like a normal page, so the app goes full screen. | Mobile web |
...
| 12 | Ideally most of our top apps should have an app icon and should work. Maybe even separate them from not working/demo-only apps (maybe even hide them, with a possibility to show on expand). | Curate: top apps get icons + verified working; demo/broken apps collapsed behind "show more". **DECIDE** hide vs. demote. | Discover |
| 13 | We should block emails containing such trickery we are now using with "+test" for distinct accounts. | **DECIDE**: rec normalize plus-addressing for dedupe (`a+x@` counts as `a@`) rather than blocking, with an allowlist for internal testing. | Auth |
| 14 | On other apps, the URL on Safari at the bottom of the screen shrinks when you scroll, here it doesn't, but would be nice if it did here too. | Same fix as #7, Safari case: let the browser bar shrink on scroll. | Mobile web |
...
| 16 | We should decide what the start page on the app should look like – on web we were saying the landing page, but maybe should be something else on mobile. | **DECIDE** mobile start page. Rec: signed-in → dashboard; landing only when signed out. | Mobile app |
...
| 19 | Can we make the confirm your email step happen as a part 2 to joining the waitlist? So after you click join waitlist, it just says great you're on the waitlist, and then confirming your email is a part 2? | **DECIDE**: rec yes — instant "you're on the waitlist", email confirmation as an explicit step 2. | Waitlist |
...
| 22 | Confirm your email screen in the waitlist flow is not optimal. Text confusing. There are 2 ways of validating an email address. Should be discussed. | **DECIDE** the one canonical verification method (link vs. code), then rewrite the screen around it (with #19, #31, #39). | Waitlist |
...
| 25 | "Only let me in when at least one person from my link gets in too". Should we have this? | **DECIDE**: rec no for v1 — niche preference, complicates queue logic; revisit later. | Waitlist |
...
| 27 | Maybe instead of just one "Want in sooner / answer them now", we should have 3 boxes for things that can move you up the list? That might be easier for users to digest & also see the summary of their progress doing things on the list. | **DECIDE**: rec yes — ~3 "move up the list" cards (answers, socials, referrals) with done-states doubling as a progress summary. | Waitlist |
...
| 43 | App dropdown menu confusing. | ❓ Simplify the app dropdown: fewer, clearly named entries with icons (with #110, #112). See question below. | Core UI |
| 44 | Improve menu confusing. | ❓ Restructure Improve around clear states — nothing running / running / done (with #93–#96, #107). See question below. | Dev screen |
...
| 46 | Discover / Browse all apps -> Add button confusing. Not sure it makes sense for endusers, may be we have to rename it. | Rename to something self-explanatory ("Add to my apps"); likely removed anyway if #57 lands. | Discover |
...
| 52 | I don't see versions anymore. | ❓ Restore visible version history where it previously appeared. See question below. | Dev screen |
...
| 54 | Create app says: ask admin to enable app creation. Change to 2 allowed apps but still not able to create an app. Works well for admin user. | Fix: raising a user's app allowance must actually enable creation for non-admin users. | Create/fork |
...
| 57 | Clicking an app in discover opens the app immediately, maybe instead it should go to the browse app page for that app, where you can add it, see info about it, etc? And then we can remove the + buttons on all of the apps there too. | **DECIDE**: rec yes — tap opens the app's detail page (info, add) instead of launching; then drop the per-card + buttons. | Discover |
| 58 | The challenges area, we have 9 initial onboarding challenges – feels like too many, maybe we should have like 3? And then you unlock a few persistent challenges that are there now (like prove your identity), as well as the weekly ones. | **DECIDE**: rec cut onboarding to ~3, then unlock persistent + weekly challenges. | Challenges |
...
| 60 | When changing username, I just get "saving…", changing username should gray out during that time. | Gray out the field and button while saving is in flight. | Settings |
...
| 73 | Would be nice to have a pin button on game corner games to save them / push them to the top. | Add pin/favorite that keeps chosen games at the top. | Game Corner |
...
| 89 | Default code editor should be openrouter with GLM 5.3 Flash. | **DECIDE**: rec verify quality/cost on sample builds first, then make OpenRouter GLM 5.3 Flash the default if it holds. | Dev screen |
...
| 108 | We should be able to see somewhere our app limit and how long until it is reset. | Show app limit, usage, and reset time (create dialog and/or settings). | Create/fork |
...
| 114 | Asking for notifications permissions every time a change is proposed or started. | Ask for permission once; remember the choice, never re-prompt per proposal. | Core UI |
...
- **#43** — App dropdown: what confused — the labels, the contents, or finding it at all?
- **#44** — Improve menu: what confused most — entry point, states, or contents?
...
- **#52** — Where did you used to see versions (dev screen, app page, elsewhere)?
